...
(function() {
    ...
    const LOCATIONS = [
        { id: 'kushimoto', name: '串本', nameEn: 'Kushimoto', lat: 33.470, lng: 135.696 },
        { id: 'ogasawara', name: '小笠原', nameEn: 'Ogasawara', lat: 27.083, lng: 142.179 },
        { id: 'amami', name: '奄美大島', nameEn: 'Amami', lat: 28.105, lng: 129.160 },
        { id: 'okinawa_sesoko', name: '瀬底', nameEn: 'Sesoko', lat: 26.65, lng: 127.85 },
        { id: 'okinawa_manza', name: '万座', nameEn: 'Manza', lat: 26.515, lng: 127.830 },
        { id: 'kerama', name: '慶良間', nameEn: 'Kerama', lat: 26.186, lng: 127.374 },
        { id: 'sekisei', name: '石西礁湖', nameEn: 'Sekisei', lat: 24.337, lng: 124.035 }
    ];
    ...
    function getUVLevel(uv, isJa) {
        if (uv === null || uv === undefined) return { text: '-', class: '' };
        if (uv <= 2) return { text: isJa ? '弱い' : 'Low', class: 'uv-low' };
        if (uv <= 5) return { text: isJa ? '中程度' : 'Mod', class: 'uv-moderate' };
        if (uv <= 7) return { text: isJa ? '強い' : 'High', class: 'uv-high' };
        if (uv <= 10) return { text: isJa ? '非常に強い' : 'V.High', class: 'uv-veryhigh' };
        return { text: isJa ? '極端' : 'Extreme', class: 'uv-extreme' };
    }
    ...
    async function fetchTideData() {
        ...
        for (const loc of LOCATIONS) {
            const url = `${SUPABASE_URL}/rest/v1/tide_extremes?location=eq.${loc.id}&datetime=gte.${startISO}&datetime=lt.${endISO}&order=datetime.asc&select=datetime,tide_type,height`;
            ...
            try {
                ...
                console.error(`Error fetching tide for ${loc.id}:`, e);
                results[loc.id] = [];
            }
        }
    ...
    function getTodayTides(tideData) {
        ...
        const todayTides = tideData.filter(t => t.datetime && t.datetime.startsWith(todayStr));
        
        const high = todayTides.find(t => t.tide_type === 'high');
        const low = todayTides.find(t => t.tide_type === 'low');
        
        return {
            high: high ? formatTideTime(high.datetime) : '-',
            low: low ? formatTideTime(low.datetime) : '-'
        };
    }
    
    function formatTideTime(isoString) {
        ...
        for (const loc of LOCATIONS) {
            ...
            <div class="marine-summary-card" onclick="this.classList.toggle('expanded')">
                ...
                <div class="marine-summary-row">
                    ...
                    <div class="marine-summary-item">
                        <span class="label">${isJa ? '気温' : 'Air'}</span>
                        <span class="value">${todaySummary?.airTemp?.toFixed(0) || '-'}℃</span>
                    </div>
                    <div class="marine-summary-item">
                ...
                <div class="marine-summary-row marine-summary-row-extra">
                    ...
                    <div class="marine-summary-item">
                        ...
                        <span class="value tide-value">${isJa ? '満' : 'H'}${tides.high} ${isJa ? '干' : 'L'}${tides.low}</span>
                    </div>
                </div>
